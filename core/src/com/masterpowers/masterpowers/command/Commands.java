...
public class Commands {
	...
	private final MasterPowers plugin;

	public static Set<String> invincible = new HashSet<String>();
	public static boolean debugEnabled = false;
	public static boolean isToggledForAll = false;

	public Commands(final MasterPowers plugin) {
		this.plugin = plugin;
		debugEnabled = MasterPowers.plugin.getConfig().getBoolean("debug");
		this.init();
	}
	...
	// Miscellaneous.
	public static String[] commandaliases = { "b", "mp", "masterpowers", "bending", "mtla", "tla", "powers", "bend" };

	private void init() {
		final PluginCommand masterpowers = this.plugin.getCommand("masterpowers");
		...
		masterpowers.setExecutor(exe);
		masterpowers.setTabCompleter(new BendingTabComplete());
	}
